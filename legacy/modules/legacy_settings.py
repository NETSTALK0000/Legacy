...
class LegacySettingsMod(loader.Module):
    ...
    async def weburl(self, message: Message, force: bool = False):
        ...
        if not main.legacy.web:
            main.legacy.web = core.Web(
                data_root=main.BASE_DIR,
                api_token=main.legacy.api_token,
                proxy=main.legacy.proxy,
                connection=main.legacy.conn,
            )
            await main.legacy.web.add_loader(self._client, self.allmodules, self._db)
            await main.legacy.web.start_if_ready(
                len(self.allclients),
                main.legacy.arguments.port,
                proxy_pass=main.legacy.arguments.proxy_pass,
            )
