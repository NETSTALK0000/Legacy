...
class Module:
    ...
    @legacy_watchers.setter
    def legacy_watchers(self, _):
        pass
    ...
    async def _decline(
        ...
        self._db.set(
            "legacy.main",
            "declined_joins",
            list(set(self._db.get("legacy.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
    ...
    async def request_join(
        ...
        self.legacy_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "legacy_wait_channel_approve")
    ...
    async def import_lib(
        ...
        *,
        ...
        module = f"legacy.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("legacy.main", "stats", True)
            and url is not None
