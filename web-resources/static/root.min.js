		...
		fetch("/init_qr_login", {
			...
			.then((e) => {
				...
				((document.querySelector(".qr_inner").innerHTML = ""),
					...
					t.append(document.querySelector(".qr_inner")),
							...
							.then((e) =>
								"SUCCESS" == e || "2FA" == e
									? ($("#block_qr_login").fadeOut(250),
										...
										"SUCCESS" == e && switch_block("custom_bot"),
										"2FA" == e && (show_2fa(), (qr_login = !0)),
...
function finish_login() {
	fetch("/finish_login", {
		...
		.catch((e) => {
			(error_state(),
				// error_message("Login confirmation error: " + e.toString()));
				error_message("Ошибка подтверждения входа: " + e.toString()));
		});
}
...
function show_2fa() {
	($(".auth-code-form")
		...
		$(".code-caption").html(
			...
			"Введите пароль двухфакторной аутентификации Telegram и нажмите <span style='color: #dc137b;'>Enter</span>",
		),
			...
			e.ok
				...
				: ($(".code-input").removeAttr("disabled"),
					e.text().then((e) => {
						...
						(error_state(), Swal.fire("Ошибка", e, "error"));
					}));
		})
		: void fetch("/tg_code", {
			method: "POST",
			...
			.catch((e) => {
				...
				Swal.showValidationMessage("Ошибка аутентификации: " + e.toString());
			});
}
...
function process_next() {
	...
	if ("api_id" == e)
		return (t = document.querySelector("#api_id").value).length < 4 || !isInt(t)
			? void error_state()
			: ((_api_id = parseInt(t, 10)), void switch_block("api_hash"));
	if ("api_hash" == e)
		return 32 == (t = document.querySelector("#api_hash").value).length
			? void fetch("/set_api", {
				method: "PUT",
				...
				.catch((e) => {
					(error_state(),
						error_message(
							// "Error occured while saving credentials: " + e.toString(),
		...
		((_phone = t),
			fetch("/send_tg_code", {
				...
				.catch((e) => {
					...
					(error_state(), error_message("Ошибка отправки кода: " + e.toString()));
				}));
	}
	if ("2fa" != e)
		return "custom_bot" == e
			? "" != (t = document.querySelector("#custom_bot").value) &&
				(!t.toLowerCase().endsWith("bot") || t.length < 5)
				? void Swal.fire({
					icon: "error",
					// title: "Bot username invalid",
					// text: "It must end with `bot` and be at least 5 symbols in length",
					title: "Неверное имя бота",
					text: "Имя должно оканчиваться на `bot` и быть не короче 5 символов",
				})
				: "" == t
					? void finish_login()
						...
						.then((e) =>
							"OCCUPIED" == e
								? void Swal.fire({
									...
									title: "Это имя бота уже занято!",
								})
								: void finish_login(),
						)
						.catch((e) => {
							(error_state(),
								// error_message("Custom bot setting error: " + e.toString()));
								error_message("Ошибка установки бота: " + e.toString()));
						})
			: void 0;
	...
	$(".installation input").on("keyup", (e) =>
		cnt_btn.disabled
			...
			: auth_required
				...
				: void (("Enter" !== e.key && 13 !== e.keyCode) || process_next()),
	),
	$(".code-input").on("keyup", (e) => {
		"code" == _current_block && 5 == $(".code-input").val().length
