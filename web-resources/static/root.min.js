			...
			($(".auth")
				...
				fetch("/web_auth", {
					...
					.then((t) =>
						"TIMEOUT" == t
							? (error_message(
								"Превышено время ожидания кода. Перезагрузите страницу и повторите попытку.",
							),
								void $(".auth").fadeOut(250))
							: t.startsWith("legacy_")
								? ($.cookie("session", t),
...
function show_2fa() {
	($(".auth-code-form")
		...
		$(".enter").hasClass("tgcode") && $(".enter").removeClass("tgcode"),
		$(".code-caption").html(
			"Введите облачный пароль Telegram 2FA, затем нажмите <span style='color: #dc137b;'>Enter</span>",
		),
		cnt_btn.setAttribute("current-step", "2fa"),
		$("#monkey").hide(),
		$("#monkey-close").hide().fadeIn(100),
		...
		return 32 == (t = document.querySelector("#api_hash").value).length
			? void fetch("/set_api", {
				...
				.catch((e) => {
					(error_state(),
						error_message(
							"Произошла ошибка при сохранении учетных данных: " + e.toString(),
						));
				})
			: void error_state();
	if ("phone" == e) {
		...
		((_phone = t),
			fetch("/send_tg_code", {
				...
				.catch((e) => {
					(error_state(), error_message("Отправка кода не удалась: " + e.toString()));
				}));
	}
	if ("2fa" != e)
		return "custom_bot" == e
			? "" != (t = document.querySelector("#custom_bot").value) &&
				...
				? void Swal.fire({
					...
					text: "Он должен заканчиваться на «bot» и иметь длину не менее 5 символов.",
				})
				: "" == t
					? void finish_login()
					: void fetch("/custom_bot", {
						...
						.then((e) =>
							"OCCUPIED" == e
								? void Swal.fire({
									icon: "ошибка",
									title: "Это имя пользователя бота уже занято!",
								})
								: void finish_login(),
						)
						.catch((e) => {
